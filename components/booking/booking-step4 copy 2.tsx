...
const calculateFareStructure = (
  ...
  const totalBaseAndToll = baseFare + tollTax + airportParking;
  ...
  let finalFare = totalBaseAndToll;
  ...
  if (step2.bookingType === "roundtrip") {
    ...
    if (dayDiff < 2) { // Same day or next day return
      const roundTripBase = totalBaseAndToll * 1.70;
      roundTripDiscount = roundTripBase * 0.30; // 30% discount on the 1.70x fare
      finalFare = roundTripBase - roundTripDiscount;
    } else { // 2 or more days later
      finalFare = totalBaseAndToll * 2;
    }
  }

  const paymentDiscount = Math.round(finalFare * discountRate);
  const calculatedDiscount = paymentDiscount + roundTripDiscount;
  const discountedFare = finalFare - paymentDiscount;
...
export default function BookingStep4({ nextStep, prevStep }: BookingStep4Props) {
  ...
  const effectiveDistance = (calculatedDistance || (step2?.bookingType === "local" ? 20 : 250)) * (step2?.bookingType === "roundtrip" ? 2 : 1);
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            {step2.bookingType === "roundtrip" && step2.returnDate && (
              <div>
                ...
                <p className="font-semibold dark:text-gray-100">
                  {step2.returnDate} at {step2.returnTime}
                </p>
              </div>
            )}
            <div>
